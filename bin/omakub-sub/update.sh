CHOICES=(
	"Xomakub-Geo   Update Xomakub itself and run any migrations"
...
if [[ "$CHOICE" == "<< Back"* ]] || [[ -z "$CHOICE" ]]; then
	...
	case "$INSTALLER" in
	"xomakub-geo") INSTALLER_FILE="$XOMAKUB_PATH/bin/xomakub-geo-sub/migrate.sh" ;;
	"localsend") INSTALLER_FILE="$XOMAKUB_PATH/install/desktop/app-localsend.sh" ;;
	"ollama") INSTALLER_FILE="$XOMAKUB_PATH/install/terminal/optional/app-ollama.sh" ;;
	*) INSTALLER_FILE="$XOMAKUB_PATH/install/terminal/app-$INSTALLER.sh" ;;
	esac
